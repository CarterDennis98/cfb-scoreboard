...
def draw_scheduled_game(game: GameScoreboard):
    ...
    draw.text((5, -1), f"{game.home_team.ranking + ' ' if game.home_team.ranking else ''}{game.home_team.short_name}", font=font, fill=white_fill)
    draw.rectangle([(0, 7), (3, 13)], fill=game.away_team.main_color)
    draw.text((5, 6), f"{game.away_team.ranking + ' ' if game.away_team.ranking else ''}{game.away_team.short_name}", font=font, fill=white_fill)
